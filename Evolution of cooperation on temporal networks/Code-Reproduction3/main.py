...
if __name__ == '__main__':
    ...
    for _Graph in range(DiffGraph):
        NOCs_ER = nx.erdos_renyi_graph(N, ER_p)
        NOCs_SF = nx.barabasi_albert_graph(N, m, m)
        for i in range(len(glist)):  # 每个snapshot的演化次数
            for j in range(len(blist)):  # 囚徒困境中的背叛者优势，横坐标
                for k in range(len(plist)):  # 随机删边的概率
                    playersInit()
                    yPoint[i][k][0][j] += EvolutionGameProcess(NOCs_ER, "PD", glist[i], blist[j], plist[k])  # 均为囚徒博弈
                    playersInit()
                    yPoint[i][k][1][j] += EvolutionGameProcess(NOCs_SF, "PD", glist[i], blist[j], plist[k])
                    print(_Graph, i, j, k, yPoint[i][k][0][j], yPoint[i][k][1][j])
